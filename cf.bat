...
sleep.vbs
start /b curl --resolve speed.cloudflare.com:443:%a% https://speed.cloudflare.com/__down?bytes=1000000000 -o nul --connect-timeout 5 nul --connect-timeout 5 temp/%%a -s --connect-timeout 2 --max-time 10
title 批量测速----正在测试 %%a
)
del sleep.vbs
del ip.txt
...
cls
echo 第一次测试 !a!
curl --resolve speed.cloudflare.com:443:%a% https://speed.cloudflare.com/__down?bytes=1000000000 -o nul --connect-timeout 5 tmp --connect-timeout 5 --max-time 10
if exist %FileName% (
...
cls
echo 第一次测试 !c!
curl --resolve curl --resolve speed.cloudflare.com:443:%a% https://speed.cloudflare.com/__down?bytes=1000000000 -o nul --connect-timeout 5 tmp --connect-timeout 5 --max-time 10
if exist %FileName% (
	for /f "skip=3 tokens=3" %%i in ('dir %FileName% /-C') do (
		 if %%i EQU %FileSize% (
			cls
			...
			if %%i GTR %FileSize% (
				...
				echo 第二次测试 !c!
				curl --resolve speed.cloudflare.com:443:%%a https://speed.cloudflare.com/__down?bytes=1000000000 -o tmp --connect-timeout 5 --max-time 10
				if exist %FileName% (
					for /f "skip=3 tokens=3" %%i in ('dir %FileName% /-C') do (
						if %%i EQU %FileSize% (
							cls
